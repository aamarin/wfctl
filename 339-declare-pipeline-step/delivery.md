...
|----|-------|--------------|------|----------------|
| PR 1 | T001–T037 | `wfctl/_pipeline.py` (modified), `wfctl/_declared.py` (created), `wfctl/_evidence.py` (modified), `wfctl/cli.py` (modified), `tests/conftest.py` (modified), `tests/test_declared.py` (created), `tests/test_cli_status.py` (created), `tests/test_evidence.py` (modified), `tests/test_pipeline_state_names.py` (modified), `tests/test_pipeline_commands.py` (modified), `tests/test_pipeline_payload_snapshot.py` (modified), `tests/pipeline_payload_snapshot.json` (modified) | L (12 files) | the parent branch's rename merged, so `_evidence.py` and the new names are already on `main`; T013, T029 and T037 gates green; `quickstart.md` § *Declare a pass and walk a branch through it* and § *Read wfctl's own passes* walked by hand |
