...
| C1 | Underspecification | MEDIUM | `data-model.md` § States; `spec.md` edge case 7; `tasks.md` T047 | The state table gives `done` when the predicate is satisfied and `skipped` when a claim exists, and states no precedence — read top-down it gives `done`, which is the opposite of the spec's edge case: "a pass is declared away, and a later commit produces its artifact anyway: the claim stands". No test covered it | **Applied**: T047 amended to read the claim ahead of the predicate; T043a added |
...
| E3 | Coverage gap | MEDIUM | `spec.md` FR-008 and edge case 5; `data-model.md` § Where the built-in ones live | FR-008 exists so a pass whose evidence is a heading inside another step's artifact can report correctly, and edge case 5 requires it. Both built-in passes read whole files (`architecture` a record, `design-doc` `design.md`), and `evidence` builds a file-exists predicate, so nothing in the delivered set exercises the capability. A regression to a path-only pass would fail no test | **Applied**: T017a |
