...
class Step(NamedTuple):
    ...
    rather than a frozen dataclass so the positional unpacks that predate it keep
    working — it is still a tuple.
...
def next_step_content(step: str, blocked: str | None = None) -> tuple[str, bool]:
    ...
    **A blocked step is never automatic, whatever the table says.** The table
    ...
    if row is None:
        return "", False
    return row.command, row.continuation == _AUTOMATIC
