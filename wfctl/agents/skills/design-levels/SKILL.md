...
| 4 implementation | nothing written by the gate — the mechanism is chosen with `.agents/skills/python-pattern-selection`, and only a departure from one of its constraints is written down |
...
Use `.agents/skills/python-pattern-selection` while writing the code: it names
the cheaper Python shape for each choice — a callable before a Strategy
hierarchy, a session transaction before a Unit of Work — so the mechanism is
picked against a stated pressure rather than by habit.
...
Then `verification-before-completion` and `code-review`, unchanged. Both fire
after the code exists — which is why the mechanism choice needed something of its
own in front of them.
...
cannot see it.
- Level 4 → belongs to the plan and to `speckit.tasks`, not to the design. The
  mechanism itself is chosen with `.agents/skills/python-pattern-selection`,
  which lands nothing by default; a deliberate departure from one of its
  constraints leaves a note under `<arch-root>/implementation/`, for level 3's
  reason and not a new one — `tasks.md` lives in the same gitignored `specs/`
  that a reviewer reading the PR never opens.
