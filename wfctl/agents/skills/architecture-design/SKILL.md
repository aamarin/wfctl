...
> name can be renamed mechanically, so the boundary does not need to be stable;
> it needs to be legible. Making a boundary checkable is not an abstraction:
> Yagni exempts "effort to make the software easier to modify" from its own
> rule.

**It gates nothing.** The step produces a recorded argument, not a verdict, and
