...
| `Stop` | `wfctl hook session-restart` | once the context window reaches `WFCTL_RESTART_THRESHOLD` tokens, types `/end-session restart` into the pane, then `/clear` and `/start-session` after that turn has recorded its stop — so a full session hands off before it is cleared. Holds the restart entirely while this session still has subagents out, so a fan-out's findings reach the handoff rather than the clear. Says once in the pane when it holds, finds no workmux pane, or sends a `/clear` that does not take |
