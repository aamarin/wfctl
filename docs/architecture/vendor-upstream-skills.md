...
`python-pattern-selection` is absent for a reason worth stating rather than
leaving to the default below, because its own commit message says "ported from a
third-party skill" and a later reader would go looking for the row. The source it
was drafted from was generated by an agent: there is no upstream project, no
repository, no licence and no copyright holder, so there is nothing to name on a
last line and no entry to add to `NOTICES.md`. Its constraints restate Python
...
"downloaded" for "upstream" goes looking for a row. The draft was generated by
an agent, and there is no upstream project, no licence and no copyright holder.
Its `references/source-map.md` names two Pearson books, Evans' *Domain-Driven
Design* and Vernon's *Domain-Driven Design Distilled*. That is a citation of
what the synthesis is of, not a derivation, and it reproduces no sample program
...
- 2026-09-13  amended     — `python-pattern-selection` named as wfctl's own, and
