...
engineering guidance and reproduces no sample program. Its `source-map.md` does
name Robert C. Martin's *Clean Code* (Pearson, 2008), mapping nineteen chapters
to the reference that carries each idea; that is a citation rather than a
derivation, since the ideas are not the book's to own and none of its expression
was taken. `vendor-upstream-skills` makes the same distinction, and it is
repeated here because this record is the one `wfctl arch context` projects — a
reader who finds the book named in one file and "no copyright holder" in another
has to open a third to learn they agree. This is
