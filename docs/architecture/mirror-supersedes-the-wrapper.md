...
- 2026-09-24  amended     — #473; the eight pipeline-step wrappers dropped `disable-model-invocation`, which *Considered* rejects for a wrapper whose skill shares its name. The rejection does not reach them: `speckit.plan` and `speckit-plan` differ by a dot, so no `/name` is claimed twice. `speckit-orchestrate` mirrored, and like `speckit-brainstorm` its wrapper is a dot away and collides with nothing.
- 2026-09-25  amended     — #485; `i-have-adhd` and `conversation-response-shape` left the bundle, so suppression now covers fifteen. The `i-have-adhd` case above describes a skill wfctl no longer ships, and the rule it illustrates still holds for any vendored skill that carries upstream's `disable-model-invocation`.
