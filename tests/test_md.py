...
each caller had its own. `_shape` implemented CommonMark's, `_arch` matched a
prefix; the difference only shows on a document that quotes a fenced example,
which is exactly what each of them reads. So these tests pin the rule itself
rather than any one caller's projection of it — a caller's own tests cover
shapes that reach it, and the shapes that did *not* reach it are the ones that
made the implementations disagree for a year without anyone noticing.
"""
...
def test_a_closer_carrying_an_info_string_is_content() -> None:
    ...
    The rule `_arch` did not have: it matched a prefix, so a document showing
    two fenced examples back to back had the second one's opening read as the
    first one's close.
    """
    assert _inside("````\nb\n```python\nc\n````\n") == ["b", "```python", "c"]
