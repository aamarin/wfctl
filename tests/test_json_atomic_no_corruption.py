"""Tests for write_atomic atomicity under failure."""
...
def test_atomic_write_no_leftover_tmp_on_failure(tmp_path: Path) -> None:
    """No .tmp orphan left when write_atomic fails."""
    target = tmp_path / "data.json"
    write_atomic(target, json.dumps({}, indent=2))  # create target first

    with patch("os.replace", side_effect=OSError("simulated")):
        with pytest.raises(OSError):
            write_atomic(target, json.dumps({"new": 1}, indent=2))
...
def test_atomic_write_creates_valid_json(tmp_path: Path) -> None:
    target = tmp_path / "data.json"
    write_atomic(target, json.dumps({"a": 1, "b": [1, 2, 3]}, indent=2))
