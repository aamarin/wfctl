...
- [X] T001 Rename the middle authority class from *outward-facing* to
...
- [X] T002 [P] Update the comment near `_unkeyed_issues` in `wfctl/_pipeline.py`
...
- [X] T003 (FR-001, FR-002) Add `NotifyGrant` NamedTuple and
...
- [X] T004 (FR-001, FR-006, FR-007) Add `grant_notify(agent_dir, state)` to
...
- [X] T005 (FR-010, FR-015) Add `record_notify_action(agent_dir, action, count)`
      ...
      string. Verify: T008.
- [X] T006 (FR-002, FR-015) Unit tests in `tests/test_session.py` for the
      ...
      distinct `source`.
- [X] T007 [P] Unit test that `grant_notify` writes both the file and the event,
...
- [X] T008 [P] Unit test that `record_notify_action` appends without rewriting
      prior lines, and that a malformed pre-existing line does not break the append.
- [X] T008a Enforce FR-008: a grant read on the trunk branch returns refused
      regardless of any stored value or label, in `wfctl/_session.py`. Verify: T008b.
- [X] T008b [P] Test FR-008 from the *granted* side — write `notify.json` with
...
- [X] T009 [US1] Carry `notify` and `notify_source` into `build_report` in
      `wfctl/_pipeline.py`, **present and false** when refused, never omitted
      (FR-004). Verify: T012.
- [X] T010 [US1] Print the refused console lines in `wfctl status`
      ...
      console.
- [X] T011 [US1] Gate the notifying action in **both** skills that take one:
      ...
      prose.
- [X] T012 [US1] [P] Test in `tests/` that `--json` carries both keys in every
      refused state, and that a consumer can tell refusal from a wfctl too old
      to know (FR-003's whole point).
- [X] T013 [US1] [P] Console test with `NO_COLOR` pinned: each refusal renders
      its own line, and the three are distinguishable from each other.
- [X] T014 [US1] (SC-001) Manual exercise: `uv run wfctl install-skills`, then
...
- [X] T015 [US2] Add `--allow-notify/--deny-notify` to `start_cmd` in
...
- [X] T016 [US2] Read the `authority:notify` label through the existing tracker
      `view` verb, once per run (FR-014). Skip the read where no tracker is
      configured (FR-012). Verify: T020.
- [X] T017 [US2] Print the granted console lines, naming the source — label or
      local (FR-005). Verify: T021.
- [X] T018 [US2] Call `record_notify_action` from each notifying action so the
      run reports what it did, unprompted (FR-010). Verify: T022.
- [X] T019 [US2] [P] Test the tri-state: `--allow-notify` grants,
      `--deny-notify` denies, neither leaves the stored value alone.
- [X] T020 [US2] [P] Test the resolution grid end to end, including that an
      explicit local deny beats a present label and that a missing label is not
      a deny.
- [X] T021 [US2] [P] Console test that the granted line names its source, and
      that label-granted and locally-granted render differently.
- [X] T022 [US2] (SC-002, SC-004) Manual exercise with the grant present:
...
- [X] T023 [US3] Distinguish *declined by the agent* from *refused for want of a
      grant* in the reported output (FR-011). Verify: T025.
- [X] T024 [US3] Ensure the irreversible class is refused regardless of the
      grant, and that its refusal says the class is not grantable rather than
      that a grant is missing (FR-013). Verify: T026.
- [X] T025 [US3] [P] Test that the two refusal reasons render distinctly.
- [X] T026 [US3] [P] Test that no grant value makes an irreversible action
...
- [X] T027 [P] Update `AGENTS.md` if the grant changes anything an agent
...
- [X] T029 Run the review panel unattended before opening the PR — it is
      description-triggered, so an unattended run skips it silently.
- [X] T030 `uv run wfctl verify`, then open the PR against
